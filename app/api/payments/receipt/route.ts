    ...
    if (revert_balance) {
      try {
        ...
        if (student && !fetchErr) {
          ...
          if (clear_all && Array.isArray(receipts_to_revert) && receipts_to_revert.length > 0) {
            let totalTuitionRevert = 0;
            let totalFineRevert = 0;

            for (const item of receipts_to_revert) {
              const amt = Number(item.amount_paid || 0);
              if (item.fee_type === "fine") {
                totalFineRevert += amt;
              } else {
                totalTuitionRevert += amt;
              }
            }

            newPaid = Math.max(0, currentPaid - totalTuitionRevert);
            newDue = currentDue + totalTuitionRevert;
            newFine = currentFine + totalFineRevert;
          } else if (amount_paid && Number(amount_paid) > 0) {
            const amount = Number(amount_paid);
            if (fee_type === "fine") {
              newFine = currentFine + amount;
            } else {
        ...
        console.warn("Database sync note on receipt delete:", dbErr);
      }
    }

    return NextResponse.json({
      success: true,
      message: clear_all ? "All receipts cleared." : "Receipt deleted successfully.",
      receipt_id,
      student: updatedStudent,
    });
  } catch (error) {
